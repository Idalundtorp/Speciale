# -*- coding: utf-8 -*-
"""
Created on Mon Jan  2 14:42:03 2023
...
def temp_profile(self, plot=False):
    ...
    if plot==True:
        ...
        plt.plot(np.array([self.Tp_ice.flatten(), self.T_sii]), ice_thick,'.-')
        plt.ylim([-0.1,0.1])
        plt.grid()
...
class SMRT_simulations(data):
    ...
    def __init__(self, icetype, snowpit_data, directory_path):
        ...
        self.sd_sal_sim = [0.01*PSU, 4*PSU]
    ...
    def simulated_data(self, num_simulations=10, save=True, plot=True, rough=False):
        ...
        for num in range(num_simulations):
                ...
                snowpack= make_snowpack(thickness=sd_sim,
                                             ...
                                             density=self.sd_rho_sim, 
                                             corr_length=self.sd_lex_sim, 
                                             temperature = Tsnow,
                                             salinity=self.sd_sal_sim)
                ...
                if rough==True:
                    ...
                    # print('printing', self.tcwv[num],res,Tice, self.m)
                    [TB_AMSR_V,TB_AMSR_H, Gamma_rs_H, Gamma_rs_V, QH, QV] = roughness(self.tcwv[num],res,Tice, m=self.m)
                ## append final values
                self.sd_all.append(sd_sim)
                self.sd_rho_all.append(self.sd_rho_sim[:nl_snow])
                self.sd_lex_all.append(self.sd_lex_sim[:nl_snow])
                self.sd_sal_all.append(self.sd_sal_sim[:nl_snow])
                self.Tsnow_all.append(Tsnow)
                self.Tice_all.append(Tice)
                self.sit_all.append(sit)
                self.slope_all.append(self.slope)
                ## append output
        ...
        if save==True:
            ...
            with open(os.path.join(datapath,'AMSR2_TB_simulated.npy'), 'wb') as f:
...
print('mean is:', np.mean(SIM_FYI.sd, axis=0))
print('mean is:', np.mean(SIM_FYI.TbV_AMSR, axis=0))
print('mean is:', np.mean(SIM_FYI.TbH_AMSR, axis=0))
ns = len(SIM_FYI.sd)
SIM_FYI.simulated_data(num_simulations=ns, save=True, rough=True)
